...
contract AncientMortgageProtocol is ERC721, Ownable, ReentrancyGuard, Pausable {
    ...
    function calculateMonthlyPayment(uint256 principal) public pure returns (uint256) {
        ...
        uint256 monthlyPayment = principal.mul(monthlyRate).mul((1000.add(monthlyRate)).pow(numPayments)).div(
            (1000.add(monthlyRate)).pow(numPayments).sub(1000.pow(numPayments))
        );
